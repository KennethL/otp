...
gen_decode(Erules,Typename,Type) when is_record(Type,type) ->
	...
	{constructed,bif} ->
	    ...
	    emit([FunctionName,"'(JsonTerm",ObjFun,") ->",nl]),
...
gen_encode_field_call(ObjName,FieldName,Type) ->
	...
	{primitive,bif} ->            %tag should be the primitive tag
	    gen_encode_prim(jer,Def,"Val"),
	    [];
	{constructed,bif} ->
            Name = lists:concat([ObjName,'_',FieldName]),
	    emit(["   ",{asis,enc_func(Name)},"(Val)"]),
...
gen_decode_objectfields(ClassName,[{typefield,Name,OptOrMand}|Rest],
	...
	case {get_object_field(Name,ObjectFields),OptOrMand} of
		...
		gen_decode_default_call(ClassName,Name,"JsonTerm",DefaultType);
	    {{Name,TypeSpec},_} ->
		%% A specified field owerwrites any 'DEFAULT' or
		%% 'OPTIONAL' field in the class
		EmitFuncClause("Bytes"),
		emit_tlv_format("Bytes"),
		gen_decode_field_call(ObjName,Name,"JsonTerm",TypeSpec)
	end,
    case more_genfields(Rest) of
	true ->
	    emit([";",nl]);
...
gen_decode_default_call(ClassName,FieldName,Bytes,Type) ->
	...
	{primitive,bif} ->
	    ...
	    [];
	#'Externaltypereference'{module=CurrentMod,type=Etype} ->
	    emit(["   'dec_",Etype,"'(",Bytes,")",nl]),
	    [];
	#'Externaltypereference'{module=Emod,type=Etype} ->
	    emit(["   '",Emod,"':'dec_",Etype,"'(",Bytes,")",nl]),
	    []
    end.
%%%%%%%%%%%
